...
{
    public record GetDailyTankReconcillationQuery ( DateTime Date): IRequest<List<TankReconcillationDTO>>;
    ...
    public class GetDailyTankReconcillationQueryHandler : IRequestHandler<GetDailyTankReconcillationQuery, List<TankReconcillationDTO>>
    {
        ...
        public async Task<List<TankReconcillationDTO>> Handle(GetDailyTankReconcillationQuery request, CancellationToken cancellationToken)
        {
            try
            {
                return _mapper.Map<List<TankReconcillationDTO>>(await _context.Dailytankreconciliations.Where(x => x.ReconciliationDate.Date == request.Date.Date).ToListAsync(cancellationToken));
            }
