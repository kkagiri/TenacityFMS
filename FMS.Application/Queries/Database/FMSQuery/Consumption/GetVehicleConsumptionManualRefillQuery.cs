...
namespace FMS.Application.Queries.Database.FMSQuery.Consumption {
    public record GetVehicleConsumptionManualRefillQuery (DateTime StartDate, DateTime EndDate) : IRequest<List<ManualDispenseConsumptionDTO>>;

    public class GetVehicleConsumptionManualRefillQueryHandler : IRequestHandler<GetVehicleConsumptionManualRefillQuery, List<ManualDispenseConsumptionDTO>> {

        private readonly GpsdataContext _context;
        private readonly ILogger<GetVehicleConsumptionManualRefillQueryHandler> _logger;

        public GetVehicleConsumptionManualRefillQueryHandler (GpsdataContext context, ILogger<GetVehicleConsumptionManualRefillQueryHandler> logger) {
            _context = context;
            _logger = logger;
        }
        public async Task<List<ManualDispenseConsumptionDTO>> Handle (GetVehicleConsumptionManualRefillQuery request, CancellationToken cancellationToken) {
            try {
