...
namespace FMS.Application.Infrastructure.DistCacheTracker {
    ...
    /// <summary>
    /// The AuthorizationStateTracker is responsible for tracking the real-time authorization state of each pump/nozzle in the fueling system.
    /// It ensures that only one fueling process is active per nozzle at a time, records which tag/vehicle is authorized, the allowed amount, and the transaction ID.
    /// This service is used by both UI-driven and hardware-driven (PTS) workflows to coordinate fueling operations, prevent conflicts, and maintain auditability.
    /// The state is stored in a distributed cache (e.g., Redis) for resilience and scalability.
    /// </summary>
    public class AuthorizationStateTracker : IAuthorizationStateTracker {
        ...
        private string GetCacheKey (string deviceId, int nozzleId) => $"auth:{deviceId}:{nozzleId}";
        ...
        public async Task<AuthState> GetAuthorizationStateByNozzle (string deviceId, int nozzleId) {
            try {
                var key = $"auth:{deviceId}:{nozzleId}";
                var jsonData = await _cache.GetStringAsync (key);
                if (string.IsNullOrEmpty (jsonData)) return null!;
                return JsonSerializer.Deserialize<AuthState> (jsonData) !;
            } catch (Exception ex) {
                _logger.LogError (ex, "Error getting authorization state for {DeviceId}:{NozzleId}", deviceId, nozzleId);
                throw;
            }
        }
    }
