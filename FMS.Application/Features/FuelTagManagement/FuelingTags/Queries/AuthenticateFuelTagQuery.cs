...
namespace FMS.Application.Features.FuelTagManagement.FuelingTags.FuelingTags.Queries
{
    public record AuthenticateFuelTagQuery (string fuelTagName) : IRequest<AuthenticateTagResult>;

    public class AuthenticateTagQueryHandler : IRequestHandler<AuthenticateFuelTagQuery, AuthenticateTagResult> {
        ...
        public async Task<AuthenticateTagResult> Handle (AuthenticateFuelTagQuery request, CancellationToken cancellationToken) {
            try {

                var tag = await _context.FuelTags.Include (t => t.FuelRuleSet).ThenInclude (rs => rs.Rules).FirstOrDefaultAsync (x => x.Name == request.fuelTagName);

                if (tag == null)
                    return new AuthenticateTagResult (false, null, 0, false);

                bool isMasterTag = tag.IsMaster.GetValueOrDefault ( 0) == 1;

                if (!tag.IsEnabled.GetValueOrDefault (true))
                    return new AuthenticateTagResult (false, tag, 0, isMasterTag);

                var dailyFuelIssued = await _mediator.Send (new GetDailyFuelIssuedForFuelTagQuery (tag, DateTime.Now), cancellationToken);
                ...
                foreach (var ruleData in tag.FuelRuleSet!.Rules.Where (r => r.IsActive)) {
                    ...
                    if (ruleData.DailyLimitLiter.HasValue || ruleData.MonthlyLimitLiter.HasValue) {
                        ...
                        _logger.LogWarning ($"Rule ID {ruleData.Id} for Tag {request.fuelTagName} is of an unknown or base type and cannot be evaluated.");
                        continue;
                    }
    ...
    public record AuthenticateTagResult (bool IsAuthenticated, FuelTag Tag, decimal dose, bool IsMasterTag);
}