/**
 * File: WidgetFactoryService.cs
 * Purpose: Validates widget configuration, normalizes widget requests, and bridges widgets to dashboard data sources.
 * Dependencies: IDataSourceManager, WidgetFactoryCoordinator, dashboard widget DTOs
 * Last Modified: 2026-03-25
 *
 * Key Functions:
 * - GetWidgetDataAsync(): Processes widget configuration and loads raw/transformed widget payloads.
 * - CreateMetricRequest(): Preserves widget filter and settings metadata for downstream data-source execution.
 */
using System;
    ...
    public class WidgetFactoryService : IWidgetFactoryService
    {
        ...
        private DashboardMetricRequestDto CreateMetricRequest(WidgetDataRequest request, WidgetDataProcessingResult processingResult)
        {
            return new DashboardMetricRequestDto
            {
                ...
                AggregationType = processingResult.AggregationType,
                GroupBy = ExtractGroupBy(request.Settings, processingResult.ProcessedSettings),
                Granularity = ExtractGranularity(request.Settings, processingResult.ProcessedSettings)
            };
        }
        ...
        private string ExtractGroupBy(
            Dictionary<string, object> settings,
            Dictionary<string, object> processedSettings)
        {

            if (processedSettings.TryGetValue("groupBy", out var processedGroupBy) && processedGroupBy != null)
            {
                return processedGroupBy.ToString() ?? string.Empty;
            }

            if (settings.TryGetValue("groupBy", out var requestedGroupBy) && requestedGroupBy != null)
            {
                return requestedGroupBy.ToString() ?? string.Empty;
            }

            return string.Empty;
        }

        private string? ExtractGranularity(
            Dictionary<string, object> settings,
            Dictionary<string, object> processedSettings)
        {

            if (processedSettings.TryGetValue("granularity", out var processedGranularity) && processedGranularity != null)
            {
                return processedGranularity.ToString();
            }

            if (settings.TryGetValue("granularity", out var requestedGranularity) && requestedGranularity != null)
            {
                return requestedGranularity.ToString();
            }
