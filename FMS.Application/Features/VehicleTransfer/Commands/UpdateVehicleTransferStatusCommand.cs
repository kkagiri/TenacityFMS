...
public class UpdateVehicleTransferStatusCommandHandler : IRequestHandler<UpdateVehicleTransferStatusCommand, FMSResponse<bool>>
{
    ...
    public async Task<FMSResponse<bool>> Handle(UpdateVehicleTransferStatusCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Validate status
            var validStatuses = new[] { "Pending", "InTransit", "Completed", "Cancelled" };
            ...
            if (request.Status.Equals("Completed", StringComparison.OrdinalIgnoreCase) && !transfer.ArrivalTime.HasValue)
            {
            ...
            _logger.LogInformation("Vehicle transfer {TransferId} status updated to {Status}", request.TransferId, request.Status);
            ...
            return FMSResponse<bool>.Success(true, $"Transfer status updated to {request.Status}");
        }
        catch (Exception ex)
        {
