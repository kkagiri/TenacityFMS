...
public record UpdateIssueStatusCommand (Issuestatus Status) : IRequest<Unit>;
...
public class UpdateIssueStatusCommandHandler : IRequestHandler<UpdateIssueStatusCommand, Unit> {
    ...
    public async Task<Unit> Handle (UpdateIssueStatusCommand request, CancellationToken cancellationToken) {
        try {
            Issuestatus? entity = await _context.Issuestatuses.FindAsync (request.Status.Id, cancellationToken);

            if (entity == null) {
                _logger.LogWarning ("Issue status with ID: {Id} not found", request.Status.Id);
                return Unit.Value;
            }
