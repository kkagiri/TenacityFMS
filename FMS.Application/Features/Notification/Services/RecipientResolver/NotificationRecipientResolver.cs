...
namespace FMS.Application.Features.Notification.Services.RecipientResolver {
    public class NotificationRecipientResolver : INotificationRecipientResolver {
        ...
        public async Task<List<NotificationRecipientDto>> ResolveRecipientsAsync (
            ...
            // 1. Explicit Recipients (DEPRECATED - only for backward compatibility)
            // ⚠️ IMPORTANT: This approach is deprecated for internal business functions
            // Internal notifications should use TriggerSource → Business Function Groups instead
            if (request.Recipients?.Any () == true && request.NotificationPolicyId.HasValue) {
                // Only process hardcoded recipients for policy-based notifications, not internal business functions
                _logger.LogWarning ("Hardcoded recipients provided for notification with PolicyId {PolicyId}. Consider using policy recipient configuration instead.",
                ...
                foreach (NotificationRecipientDto explicitRecipient in request.Recipients) {
                    ...
                    recipients.Add (new NotificationRecipientDto {
                        UserId = explicitRecipient.UserId,
                            DeliveryMethods = explicitRecipient.DeliveryMethods,
                            PriorityOverride = explicitRecipient.PriorityOverride,
                            ResolvedFrom = "Legacy-Hardcoded"
                    });
                    addedUserIds.Add (explicitRecipient.UserId);
                }
            } else if (request.Recipients?.Any () == true && request.NotificationPolicyId == null) {
                // Log warning for internal business functions using hardcoded recipients
                _logger.LogWarning ("Internal business function '{TriggerSource}' is using hardcoded recipients. " +
                    "This is deprecated. Please configure Business Function Groups instead.", request.TriggerSource);
            }
