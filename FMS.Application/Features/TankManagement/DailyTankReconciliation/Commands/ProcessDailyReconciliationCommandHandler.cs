...
// //Cursor - Handler for processing daily tank reconciliation with Python script logic converted to C#
...
//                 .Where (tvh => tvh.Timestamp.Date == date.Date);
...
//                 .GroupBy (tvh => tvh.TankId)
//                 .Select (g => new {
//                     TankId = g.Key,
//                         TransactionDate = date,
//                         FirstTimestamp = g.Min (tvh => tvh.Timestamp),
//                         LastTimestamp = g.Max (tvh => tvh.Timestamp),
//                         TotalDispense = g.Where (tvh => tvh.ChangeReason == VolumeChangeReasonEnum.Dispense)
