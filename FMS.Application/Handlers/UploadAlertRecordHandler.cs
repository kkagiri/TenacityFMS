...
using FMS.Application.Common; //Cursor - Add FMSResponse import
...
namespace FMS.Application.Handlers
{
    ...
    public class UploadAlertRecordHandler : IPacketHandler
    {
            ...
            IConnectionMultiplexer redisConnection)
        {
            ...
            _alarmHandlerService = alarmHandlerService ?? throw new ArgumentNullException(nameof(alarmHandlerService));
        ...
        private async Task ProcessAlarmNotificationAsync(string deviceId, AlertRecordDto alertDto, Alarm alarm)
        {
            try
            {
                ...
                var device = await _context.Devices.FirstOrDefaultAsync(d => d.DeviceImei.ToString() == deviceId); //Cursor: Device uses DeviceImei property, not Id
                ...
                var alarmRequest = new CreateAlarmNotificationRequest
                {
                    ...
                    // TriggerSource = "PTS", //Cursor: Property doesn't exist on CreateAlarmNotificationRequest
                    TriggeredBy = "System",
                    SiteId = tank?.SiteId, //Cursor: Device entity doesn't have SiteId property, only use tank's SiteId
                    DeviceId = device?.DeviceImei, //Cursor: Use DeviceImei instead of Id property
                    TankId = tank?.Id,
                    Data = new
                    {
                        AlertCode = alertDto.Code,
