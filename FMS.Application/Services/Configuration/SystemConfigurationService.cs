    ...
    public class SystemConfigurationService : ISystemConfigurationService
    {
        ...
        public async Task<int> GetWebSocketTimeoutSecondsAsync(CancellationToken cancellationToken = default)
        {
            return await GetConfigurationValueAsync(
                SystemConfiguration.DB_CONFIG_WEBSOCKET_TIMEOUT_KEY,
                _settings.DeviceActivity.WebSocketTimeoutSeconds,
                SystemConfiguration.DEFAULT_WEBSOCKET_TIMEOUT_SECONDS,
                cancellationToken);
        }
        ...
        {
            return await GetConfigurationValueAsync(
                SystemConfiguration.DB_CONFIG_HTTP_TIMEOUT_KEY,
                _settings.DeviceActivity.HttpTimeoutSeconds,
                SystemConfiguration.DEFAULT_HTTP_TIMEOUT_SECONDS,
                cancellationToken);
        }
        ...
        {
            return await GetConfigurationValueAsync(
                SystemConfiguration.DB_CONFIG_DEVICE_ACTIVITY_INTERVAL_KEY,
                _settings.DeviceActivity.CheckIntervalSeconds,
                SystemConfiguration.DEFAULT_DEVICE_ACTIVITY_CHECK_INTERVAL_SECONDS,
                cancellationToken);
        }
        ...
        public async Task<TimeSpan> GetWorkStartTimeAsync(CancellationToken cancellationToken = default)
        {
            var workStartTimeStr = await GetConfigurationValueAsync(
                SystemConfiguration.DB_CONFIG_WORK_START_TIME_KEY,
                _settings.WorkSchedule.WorkStartTime,
                "06:00",
                cancellationToken);

            return TimeSpan.TryParse(workStartTimeStr, out var time) ? time : SystemConfiguration.DEFAULT_WORK_START_TIME;
        }
        ...
        public async Task<TimeSpan> GetWorkEndTimeAsync(CancellationToken cancellationToken = default)
        {
            var workEndTimeStr = await GetConfigurationValueAsync(
                SystemConfiguration.DB_CONFIG_WORK_END_TIME_KEY,
                _settings.WorkSchedule.WorkEndTime,
                "22:00",
                cancellationToken);

            return TimeSpan.TryParse(workEndTimeStr, out var time) ? time : SystemConfiguration.DEFAULT_WORK_END_TIME;
        }

        public async Task<string> GetTimezoneAsync(CancellationToken cancellationToken = default)
        {
            return await GetConfigurationValueAsync(
                SystemConfiguration.DB_CONFIG_TIMEZONE_KEY,
                _settings.WorkSchedule.Timezone,
                SystemConfiguration.DEFAULT_TIMEZONE,
                cancellationToken);
        }
