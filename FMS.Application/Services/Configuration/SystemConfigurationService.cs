...
namespace FMS.Application.Services.Configuration
{
    ...
    public class SystemConfigurationService : ISystemConfigurationService
    {
        ...
        /// <summary>
        /// Gets boolean configuration value with priority: Database > Default
        /// </summary>
        private async Task<bool> GetBoolConfigurationValueAsync(string dbKey, bool defaultValue, CancellationToken cancellationToken)
        {
            string? dbValue = await GetConfigurationValueAsync(dbKey, cancellationToken);

            if (!string.IsNullOrEmpty(dbValue))
            {
                if (bool.TryParse(dbValue, out bool parsedValue))
                {
                    return parsedValue;
                }
                // Handle "1" / "0" values from database
                if (dbValue == "1" || dbValue.Equals("true", StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
                if (dbValue == "0" || dbValue.Equals("false", StringComparison.OrdinalIgnoreCase))
                {
                    return false;
                }
            }
        ...
        /// <summary>
        /// Gets decimal configuration value with priority: Database > Default
        /// </summary>
        private async Task<decimal> GetDecimalConfigurationValueAsync(string dbKey, decimal defaultValue, CancellationToken cancellationToken)
        {
            string? dbValue = await GetConfigurationValueAsync(dbKey, cancellationToken);

            return !string.IsNullOrEmpty(dbValue) && decimal.TryParse(dbValue, out decimal parsedValue)
                ? parsedValue
                : defaultValue;
        }
        #endregion
        ...
        public async Task<bool> GetPtsUpdateTankVolumeFromBookKeepingAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_UPDATE_TANK_VOLUME_FROM_BOOKKEEPING_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_UPDATE_TANK_VOLUME_FROM_BOOKKEEPING,
                cancellationToken);
        }

        public async Task<bool> GetPtsUsePtsProbeReadingsAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_USE_PTS_PROBE_READINGS_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_USE_PTS_PROBE_READINGS,
                cancellationToken);
        }
        ...
        {
            return await GetConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_VOLUME_SOURCE_PRIORITY_KEY,
                0, // No settings override
                SystemConfigurationConstants.DEFAULT_PTS_VOLUME_SOURCE_PRIORITY,
                cancellationToken);
        }

        public async Task<bool> GetPtsAutoCreateLedgerEntriesAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_AUTO_CREATE_LEDGER_ENTRIES_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_AUTO_CREATE_LEDGER_ENTRIES,
                cancellationToken);
        }

        public async Task<bool> GetPtsCheckForDuplicateManualEntriesAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_CHECK_FOR_DUPLICATE_MANUAL_ENTRIES_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_CHECK_FOR_DUPLICATE_MANUAL_ENTRIES,
                cancellationToken);
        }

        public async Task<decimal> GetPtsDuplicateVolumeToleranceAsync(CancellationToken cancellationToken = default)
        {
            return await GetDecimalConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_DUPLICATE_VOLUME_TOLERANCE_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_DUPLICATE_VOLUME_TOLERANCE,
                cancellationToken);
        }

        public async Task<bool> GetPtsAutoReconcileTankVolumesAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_AUTO_RECONCILE_TANK_VOLUMES_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_AUTO_RECONCILE_TANK_VOLUMES,
                cancellationToken);
        }

        public async Task<int> GetPtsReconciliationFrequencyMinutesAsync(CancellationToken cancellationToken = default)
        {
            return await GetConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_RECONCILIATION_FREQUENCY_MINUTES_KEY,
                0,
                SystemConfigurationConstants.DEFAULT_PTS_RECONCILIATION_FREQUENCY_MINUTES,
                cancellationToken);
        }

        public async Task<decimal> GetPtsMaxVolumeDiscrepancyThresholdAsync(CancellationToken cancellationToken = default)
        {
            return await GetDecimalConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_MAX_VOLUME_DISCREPANCY_THRESHOLD_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_MAX_VOLUME_DISCREPANCY_THRESHOLD,
                cancellationToken);
        }

        public async Task<int> GetPtsDiscrepancyActionAsync(CancellationToken cancellationToken = default)
        {
            return await GetConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_DISCREPANCY_ACTION_KEY,
                0,
                SystemConfigurationConstants.DEFAULT_PTS_DISCREPANCY_ACTION,
                cancellationToken);
        }

        public async Task<bool> GetPtsEnableFuelRulesCheckAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_ENABLE_FUEL_RULES_CHECK_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_ENABLE_FUEL_RULES_CHECK,
                cancellationToken);
        }

        public async Task<bool> GetPtsEnableFuelCapacityValidationAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
                SystemConfigurationConstants.DB_CONFIG_PTS_ENABLE_FUEL_CAPACITY_VALIDATION_KEY,
                SystemConfigurationConstants.DEFAULT_PTS_ENABLE_FUEL_CAPACITY_VALIDATION,
                cancellationToken);
        }

        public async Task<bool> GetPtsEnableGPSFuelLevelCheckAsync(CancellationToken cancellationToken = default)
        {
            return await GetBoolConfigurationValueAsync(
