 ...
 * Purpose: Executes PTS commands over Redis/WebSocket or HTTP and normalizes device responses for application services.
 * Dependencies: RedisCommandService, IDeviceHttpCommandPusher, IDeviceValidator, Newtonsoft.Json.Linq
 * Last Modified: 2026-03-23
 *
 * Key Functions:
 * - ExecuteCommandAsync(): Routes device commands through the active transport.
 * - ProcessPTSResponseHTTP(): Normalizes HTTP PTS packet responses.
 * - ConvertRedisResponseToPTSMessage(): Converts Redis Windows-service responses into the shared PTS message shape.
 */
using System;
...
public class CommandExecutor : ICommandExecutor
{
    ...
    private async Task<CommunicationMode> DeterminePreferredCommunicationModeAsync(string deviceId, DeviceInfoDTO deviceInfo)
    {
        ...
        { //To:Do Add this later   && deviceInfo.AllowedForDirectCommands
            ...
            try
            {
                ...
                if (packetsArray?.Count > 0)
                {
                    ...
                    if (firstPacket != null)
                    {
                        ...
                        actualPacketData = firstPacket["Data"]; // Preserve object or array payloads for downstream parsing
