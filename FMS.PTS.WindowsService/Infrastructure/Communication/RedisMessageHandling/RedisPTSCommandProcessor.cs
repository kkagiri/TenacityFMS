/**
 * File: RedisPTSCommandProcessor.cs
 * Purpose: Consumes Redis PTS commands, forwards them to connected devices, and publishes device responses.
 * Dependencies: StackExchange.Redis, IPTSConnectionManager, Newtonsoft.Json
 * Last Modified: 2026-03-24
 *
 * Key Functions:
 * - Start(): Subscribes to the Redis command channel.
 * - HandlePTSCommand(): Validates connectivity, dispatches the PTS command, and publishes the result.
 * - BuildPTSMessage(): Wraps Redis command data into the PTS protocol envelope.
 */
