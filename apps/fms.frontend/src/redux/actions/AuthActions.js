...
import { getTenantContextFromToken } from '../../utils/jwtUtils';
import {
    LOGIN_SUCCESS,
    LOGIN_FAILURE,
    LOGOUT,
...
/**
 * Reads tenant_id / tenant_kind / parent_tenant_id / is_platform_operator
 * from the JWT and dispatches SET_TENANT_CONTEXT. Drives ViewMode routing
 * and white-label theming. Safe for legacy tokens (defaults to 'client').
 */
const dispatchTenantContextFromToken = (dispatch, token) => {
    if (!token) return;
    const ctx = getTenantContextFromToken(token);
    dispatch({ type: SET_TENANT_CONTEXT, payload: ctx });
};
...
export const loadUser = () => async (dispatch) => {
    ...
    try {
        ...
        // Hydrate tenant context (tenant_kind etc.) from the existing JWT.
        dispatchTenantContextFromToken(dispatch, token);
...
export const signIn = (username, password) => async (dispatch) => {
    ...
    try {
        ...
        // Hydrate tenant context from the new JWT — drives ViewMode routing.
        dispatchTenantContextFromToken(dispatch, token);
