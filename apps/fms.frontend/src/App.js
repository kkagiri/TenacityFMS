...
import { fetchTenantBranding } from "./redux/actions/tenantBrandingActions";
...
function App() {
  ...
  // 3-Audience white-label: fetch tenant branding once auth resolves so the
  // M365 design tokens (--m365-primary etc.) reflect the tenant's accent
  // before the layout chrome renders.
  useEffect(() => {
    if (isAuthenticated && user) {
      dispatch(fetchTenantBranding());
    }
