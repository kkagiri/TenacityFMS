...
const ExpectedAverageForm = ({ vehicle, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const loadExpectedAverages = async () => {
      try {
        setIsLoading(true);
        const response = await dispatch(fetchExpectedAvgbyVehicle(vehicle.vehicleId));

        if (response.success) {
          // Handle the response data structure
          if (response.data && Array.isArray(response.data)) {
            setHistoricalData(response.data || []);
            // Get the most recent entry as current average
            const current = response.data.length > 0 ? response.data[0] : null;
  ...
  const handleUpdateAverage = async () => {
    try {
      ...
      await dispatch(updateExpectedAvg({
      ...
      // The action returns the response directly or throws an error
      notify('Expected average updated successfully', 'success', 3000);
      if (onSuccess) {
        onSuccess(formData.expectedKmPerLiter); // Pass the new expected average
      }
    } catch (error) {
          ...
          <div className="tw-mb-6 tw-p-4 tw-bg-blue-50 tw-rounded-lg tw-border tw-border-blue-200">
            ...
            <div className="tw-grid tw-grid-cols-3 tw-gap-4 tw-text-sm">
              ...
              <div>
               ...
               <div>
                <span className="tw-text-gray-600">Model:</span>
                <span className="tw-ml-2 tw-font-medium">{vehicle.model || 'N/A'}</span>
              </div>
               <div>
                <span className="tw-text-gray-600">Engine Model:</span>
                <span className="tw-ml-2 tw-font-medium">{vehicle.enginemodel || 'N/A'}</span>
              </div>
            </div>
          </div>
        )}
