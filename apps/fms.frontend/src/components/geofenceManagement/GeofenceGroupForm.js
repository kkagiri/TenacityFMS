    ...
    <div className="tw-p-5 tw-space-y-5">
      <div>
        ...
        <p className="tw-mt-1 tw-text-sm tw-text-gray-600">
          Use groups to organize worksites, fueling zones, or trip-detection regions and keep memberships synced with GPSGate.
        </p>
        <div className="tw-mt-3 tw-flex tw-flex-wrap tw-gap-2">
          <span
            className={`tw-inline-flex tw-items-center tw-gap-1 tw-rounded-full tw-px-3 tw-py-1 tw-text-xs tw-font-medium ${group?.isAllowedForFueling ? "tw-bg-amber-100 tw-text-amber-800" : "tw-bg-slate-100 tw-text-slate-700"}`}
            title={group?.isAllowedForFueling ? "This group is currently used for fueling validation." : "This group is currently general-purpose only."}
          >
            <i className={`fa-light ${group?.isAllowedForFueling ? "fa-gas-pump" : "fa-layer-group"}`}></i>
            {group?.isAllowedForFueling ? "Fueling enabled" : "General purpose"}
          </span>
          <span className="tw-inline-flex tw-items-center tw-gap-1 tw-rounded-full tw-bg-blue-50 tw-px-3 tw-py-1 tw-text-xs tw-font-medium tw-text-blue-700">
            <i className="fa-light fa-circle-info"></i>
            Change fueling policy from the group grid toggle
          </span>
        </div>
      </div>
