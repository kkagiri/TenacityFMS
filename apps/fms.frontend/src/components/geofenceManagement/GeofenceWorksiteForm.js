 ...
 * Purpose: Assigns or removes a geofence from a site so it is classified for vehicle-trip worksite logic.
...
const GeofenceWorksiteForm = ({ geofence, sites = [], saving = false, onCancel, onSubmit }) => {
  ...
  return (
    <div className="tw-p-5 tw-space-y-5">
      <div>
        <h3 className="tw-text-lg tw-font-semibold tw-text-gray-900">Classify for trip detection</h3>
        <p className="tw-mt-1 tw-text-sm tw-text-gray-600">
          Link <span className="tw-font-semibold">{geofence?.name || "selected geofence"}</span> to a site or project location label so the boundary can be used as a worksite during vehicle trip classification.
        </p>
      </div>

      {geofence?.siteName && (
        <div className="tw-rounded-md tw-border tw-border-blue-200 tw-bg-blue-50 tw-p-3 tw-text-sm tw-text-blue-700">
          Currently linked to site / project location: <span className="tw-font-semibold">{geofence.siteName}</span>
        </div>
      )}

      <label className="tw-flex tw-flex-col tw-gap-1">
        <span className="tw-text-sm tw-font-medium tw-text-gray-700">Site / project location</span>
        <select
          value={siteId}
          onChange={(e) => setSiteId(e.target.value)}
          className="tw-rounded-md tw-border tw-border-gray-300 tw-px-3 tw-py-2"
        >
          <option value="">Not linked to a site / project location</option>
      ...
      <div className="tw-rounded-md tw-bg-gray-50 tw-p-3 tw-text-sm tw-text-gray-600">
        If you choose a different site or project location label, the geofence will be detached from its current site first and then linked to the new one.
      </div>
