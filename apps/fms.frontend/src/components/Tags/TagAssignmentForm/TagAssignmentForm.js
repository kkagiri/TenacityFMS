...
const TagAssignmentForm = ({ vehicle, tags = [], ruleSets = [], onClose }) => {
  ...
  // Safely get the vehicle ID
  const vehicleId = vehicle?.vehicleId;

  // Filter available tags that are either unassigned or already assigned to the current vehicle
  // Added null check before calling filter
  const availableTags =
    tags && vehicleId
      ? tags.filter((tag) => !tag.vehicleId || tag.vehicleId === vehicleId)
      : [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!assignTagResult?.success) {
        throw new Error(
          assignTagResult?.error || "Failed to assign tag to vehicle."
        );
      }

      // Assign rule set to tag if a rule set is selected
      if (formData.ruleSetId) {
        ...
        if (!assignRuleResult?.success) {
          throw new Error(
            assignRuleResult?.error || "Failed to assign rule set to tag."
          );
        }
      }

      notify(
        "Tag successfully assigned to vehicle" +
          (formData.ruleSetId ? " and rule set" : "") +
          ".",
        "success",
        3000
      );
  ...
  return (
    <div className="tag-assignment-form">
      <Form>
        ...
        <SimpleItem>
          <Label text="Select Rule Set (Optional)" />
          <SelectBox
            dataSource={ruleSets || []}
            displayExpr="name"
            valueExpr="id"
        ...
        <SimpleItem>
          <div className="button-container">
            ...
            <Button
              ...
              stylingMode="contained"
              disabled={!formData.tagId || !vehicleId}
              width="120px"
            />
          </div>
        </SimpleItem>
