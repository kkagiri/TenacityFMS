...
/**
 * Extract the tenant context (tenant_id, tenant_kind, parent_tenant_id,
 * is_platform_operator) from a JWT. Emitted by the backend
 * JwtTokenGenerator → drives ViewMode routing in App.js.
 *
 * Falls back to safe defaults for legacy tokens issued before the
 * tenant_kind claim existed:
 *   - tenantKind defaults to 'client'
 *   - isPlatformOperator defaults to false
 *
 * @param {string} token
 * @returns {{
 *   tenantId: string|null,
 *   tenantKind: 'client'|'customer'|'system',
 *   parentTenantId: string|null,
 *   isPlatformOperator: boolean
 * }}
 */
export const getTenantContextFromToken = (token) => {
  const decoded = decodeJwtToken(token);
  if (!decoded) {
    return {
      tenantId: null,
  ...
  const rawKind = (decoded.tenant_kind || '').toString().toLowerCase();
  const tenantKind =
    rawKind === 'system' || rawKind === 'customer' ? rawKind : 'client';

  return {
    tenantId: decoded.tenant_id || null,
    tenantKind,
    parentTenantId: decoded.parent_tenant_id || null,
    isPlatformOperator:
      decoded.is_platform_operator === 'true' ||
      decoded.is_platform_operator === true,
  };
};
