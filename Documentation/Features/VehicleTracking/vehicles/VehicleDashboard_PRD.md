...
class GPSGateApiService {
  ...
  async fetchTrackData(vehicleId, date) { /* Track data */ }
  async fetchVehicleMetrics(vehicleId, dateRange) { /* Metrics */ }
  async fetchMaintenanceData(vehicleId) { /* Maintenance data */ }
}
```
