...
- [ ] **T1.4** Move `IVehicleTrackingProvider` + capability interfaces from `FMS.Infrastructure/VehicleTracking/Interfaces` → `FMS.Devices.Abstractions/Tracking`. Add transitional namespace shim. ⛓ T1.1 — **PARTIAL / TRANSITIONAL COMPLETE**: legacy tracking interfaces and models were physically moved into `FMS.Devices.Tracking/Legacy/VehicleTracking/` with old namespaces preserved to break the project cycle. `FMS.Devices.Tracking` no longer references `FMS.Infrastructure`; `FMS.Infrastructure` now references `FMS.Devices.Tracking`. Final namespace/API move to Abstractions remains deferred because the legacy interface still depends on Application DTOs.
...
- [x] **T1.6** Implement `ProviderRegistry`, `ProviderFactory`, `DeviceMessageRouter`, `ProviderHealthMonitor` in `FMS.Devices.Core`. _`TenantScopedProviderResolver` folded into `ProviderFactory` + `ProviderConfigRepository` (tenant filter is automatic at the repo layer)._ ⛓ T1.2, T1.4
...
- [ ] **T1.9** Rename `VehicleProviderMappingEntity` → `DeviceProviderMappingEntity`. **Rename DEFERRED** (touches ~20 consumer files); class still named `VehicleProviderMappingEntity`.
  ...
  - [ ] **T1.9b** Class rename `VehicleProviderMappingEntity` → `DeviceProviderMappingEntity` — atomic step at the end of Phase 2.
- [ ] **T1.10** Move provider entities from `FMS.Domain/Entities/VehicleTracking` → `FMS.Domain/Entities/Devices`. Update `GpsdataContext` registrations. (Domain change — explicit user approval needed.) ⛓ T1.8, T1.9 — **DEFERRED** until after T1.9b.
...
- [ ] **T1.13** **HANDOFF — User action required**: run `dotnet ef migrations add AddDevicesTenancyAndCategory --project FMS.Persistence --startup-project FMS.WebClient`, review, then `dotnet ef database update`. New columns default to `tenant_id = '00000000-0000-0000-0000-000000000000'` (system tenant) and `device_category = 'Tracking'` so existing GPS rows back-fill automatically.
...
- [x] **T2.2** Move `FMS.Infrastructure/VehicleTracking/Providers/GPSGateProvider.cs` → `FMS.Devices.Tracking/Providers/GpsGate/GPSGateProvider.cs`. Namespace updated to `FMS.Devices.Tracking.Providers.GpsGate`; `FMS.WebClient` now references `FMS.Devices.Tracking` and legacy provider discovery loads the `FMS.Devices.Tracking` assembly explicitly. _Transitional note: provider still implements the legacy `FMS.Infrastructure.VehicleTracking.Interfaces.IVehicleTrackingProvider` until T1.4 is completed._
...
- [x] **T2.6** Replace direct `IGPSGate*Service` injections in FuelAudit, Vehicle, FusionReporting, FuelTagManagement features. ⛓ T2.2 — **TRANSITIONAL COMPLETE**: scoped grep across those feature folders returns no `IGPSGate*Service`/`IGpsGate*Service` injections. Vehicle create/update/site-auto-assignment flows now depend on provider-neutral `ITrackingDriverNameService` and `ITrackingTagTransferService`; GPSGate implementations/adapters are registered behind those contracts. Final `IVehicleTrackingProvider` capability routing remains tied to T1.4 completion because the legacy tracking provider interface still depends on Application DTOs.
...
- [x] **T3.1** Create `FMS.Devices.Fueling` project. ⛓ T1.5 — added `FMS.Devices.Fueling`, registered it in both solution files, added `AddFuelingProviders()`, and created provider shells for `TechnotradePTS` and `NaftaATG`. Build validated with 0 errors. Destination folders now exist for `Providers/TechnotradePts/{Transport,Protocol,Mapping,Commands,Channels}`.
- [ ] **T3.2** Move `FMS.PTS.WindowsService/Infrastructure/Communication/WebSocket/*` → `Providers/TechnotradePts/Transport/`.
- [ ] **T3.3** Move `FMS.Application/Communication/{Connection,WebSocket,HttpPolling,Tracker}/*` → `Providers/TechnotradePts/Transport/`.
- [ ] **T3.4** Move + rename `FMS.PTS.WindowsService/Core/Protocal/*` → `Providers/TechnotradePts/Protocol/`. Fix typo `Protocal`→`Protocol` in type names too.
- [ ] **T3.5** Move PTS Redis bits (`FMS.PTS.WindowsService/Infrastructure/Communication/RedisMessageHandling/*` + PTS-specific bits of `FMS.Application/Communication/Redis/*`) → `Providers/TechnotradePts/Channels/`.
- [ ] **T3.6** For each `[PacketType]` handler in `FMS.Application/Handlers/{UploadTransactions,PumpResponse}`, create one `IPtsPacketMapper<TPacket,TCanonical>` in `Providers/TechnotradePts/Mapping/`. Mapper produces canonical message + publishes MediatR `INotification`.
- [ ] **T3.7** Move business logic from each old handler into a notification handler in `FMS.Application/Features/Devices/Fueling/{PumpTransactions,TankMeasurements,UploadStatus,Alerts}/Notifications/`.
- [ ] **T3.8** Delete `[PacketType]` attribute, `MessageHandlerRegistry`, `PTSMessageProcessor`. 🧪 3.3
- [ ] **T3.9** Move PTS command serializers from `FMS.Application/Command/PTSCommand/{ProbeCommands,PumpCommands,SystemCommands,TankCommands,UploadStatusCommands}/` → `Providers/TechnotradePts/Commands/`. Business orchestration → `Features/Devices/Fueling/PumpControl/`.
- [ ] **T3.10** Add `Providers/NaftaAtg/NaftaAtgProvider.cs` implementing `IFuelingPersistenceSink`. Subscribes to canonical pump/tank notifications, writes to Nafta DB. — **SHELL CREATED / HANDLERS PENDING**: `NaftaAtgProvider` is registered as an `IFuelingPersistenceSink` provider shell; canonical notification handlers and Nafta DB writes remain open.
- [ ] **T3.11** Move `FMS.Domain/ATGEntities/Nafta/*` → `FMS.Domain/Entities/ATG/Nafta/`. Update namespaces. (Domain change — explicit user approval needed.)
- [ ] **T3.12** Fix ATG namespace mismatch: DTOs in `FMS.Application/ModelsDTOs/PTS/*` declared in `FMS.Application.Features.ATG`. Either move physically to match or rename namespace.
- [ ] **T3.13** Mapper unit tests against captured packet samples (one test per packet type). 🧪 3.4
- [ ] **T3.14** Integration test: `NaftaAtgProvider` writes sample pump-transaction to Nafta DB on `PumpTransactionReceivedNotification`. 🧪 3.5
- [ ] **T3.15** End-to-end test rig: PTS device connects → canonical notifications fire → DB updated. 🧪 3.1

---

## Phase 4 — Application consolidation (1 sprint, depends on Phase 3)

- [ ] **T4.1** Create `FMS.Application/Features/Devices/{Provisioning,Fueling,Tracking}/` skeleton.
