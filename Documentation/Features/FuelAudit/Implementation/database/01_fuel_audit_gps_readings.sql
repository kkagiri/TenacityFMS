...
CREATE TABLE IF NOT EXISTS fuel_audit_gps_readings (
    ...
    reading_type VARCHAR(20) NOT NULL COMMENT 'Opening or closing stock reading',
    ...
    -- Data Quality
    data_quality VARCHAR(30) NOT NULL DEFAULT 'Exact' COMMENT 'Quality indicator: Exact, Interpolated, Unavailable, NoSensor, SensorNotReporting',
    data_quality_reason VARCHAR(255) NULL COMMENT 'Human-readable reason for data quality status',
    days_from_requested_date INT NULL COMMENT 'How many days back we searched to find data',
    ...
    raw_data LONGTEXT NULL COMMENT 'Raw JSON data from GPS provider (for debugging/audit)',
    ...
    -- Unique constraint: one reading per vehicle/date/type
    UNIQUE KEY uq_vehicle_date_type (vehicle_id, reading_date, reading_type),
...
-- SELECT * FROM fuel_audit_gps_readings
-- WHERE data_quality != 'Exact' ORDER BY reading_date DESC;
