...
console.log(
  "%cSignalR Connection Diagnostics",
  "color: cyan; font-weight: bold"
);
console.log(
  "%c========================================",
...
if (typeof ptsSignalRService !== "undefined") {
  ...
  console.log(
    "   Connection:",
    ptsSignalRService.connection ? "Exists" : "Not initialized"
  );
  if (ptsSignalRService.connection) {
    console.log("   Connection State:", ptsSignalRService.connection.state);
    console.log(
      "   Connection ID:",
      ptsSignalRService.connection.connectionId || "Not connected"
    );
  }
} else {
...
if (typeof ptsSignalRService !== "undefined" && ptsSignalRService.on) {
  ...
  const connRemover = ptsSignalRService.on(
    "connectionStatusChanged",
    (isConnected) => {
      console.log(
        `%c   ${isConnected ? "✓" : "✗"} Connection status changed: ${
  ...
  console.log(
    "%c   ✗ Cannot set up monitors - ptsSignalRService not available",
    "color: red"
  );
}
  ...
  if (!ptsSignalRService.isConnected) {
    ...
    ptsSignalRService
      ...
      .then(() => {
        ...
        console.log(
          "   Connection ID:",
          ptsSignalRService.connection?.connectionId
        );
      })
      .catch((err) => {
        console.log(
...
if (typeof store !== "undefined") {
  ...
  if (state.realtimeStatus) {
    ...
    console.log(
      "     - Live data enabled:",
      state.realtimeStatus.isLiveDataEnabled
    );
    console.log(
      "     - Update frequency:",
      state.realtimeStatus.updateFrequency
    );
  }
  ...
  if (state.ptsDeviceStatus) {
    ...
    if (deviceIds.length > 0) {
      console.log(
        "   Sample device:",
        deviceIds[0],
        state.ptsDeviceStatus[deviceIds[0]]
      );
    }
  }
  ...
  if (state.pumpStatus) {
    console.log(
      "   Pump Status:",
      Object.keys(state.pumpStatus).length,
      "devices"
    );
  }
} else {
...
console.log(
  ...
  process.env.REACT_APP_SIGNALR_URL || "Not set"
);
console.log(
  "   REACT_APP_API_URL:",
...
console.log(
  "\n%cMonitor console for SignalR events marked with ⚡",
  "color: lime; font-weight: bold"
);
console.log(
  "%cEvents will be logged automatically when received from backend",
  "color: white"
);
console.log("\n%cUseful commands:", "color: yellow; font-weight: bold");
console.log("  ptsSignalRService.getConnectionStatus()  - Check if connected");
console.log(
  "  ptsSignalRService.connection.state       - Get connection state"
);
console.log(
  "  ptsSignalRService.requestDeviceStatusSummary() - Request device status"
);
console.log(
  "  __signalRMonitorCleanup()                 - Remove event monitors"
);
console.log("");
