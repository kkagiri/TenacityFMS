...
public interface IDeviceMessageRouter
{
    ...
    Task PublishInTankDeliveryAsync(DeviceMessage<InTankDeliveryMessage> message, CancellationToken cancellationToken = default);
    ...
    Task PublishPumpResponseAsync(DeviceMessage<PumpResponseMessage> message, CancellationToken cancellationToken = default);
}
...
public sealed class DeviceMessageRouter : IDeviceMessageRouter
{
    ...
    {
        _logger.LogDebug("Routing in-tank delivery from {Provider}/{Device} tenant {Tenant}.",
            message.ProviderName, message.ExternalDeviceId, message.TenantId);
        return _publisher.Publish(new InTankDeliveryReceivedNotification(message), cancellationToken);
    }
    ...
    {
        _logger.LogDebug("Routing pump response {PacketType} from {Provider}/{Device} tenant {Tenant}.",
            message.Payload.PacketType, message.ProviderName, message.ExternalDeviceId, message.TenantId);
        return _publisher.Publish(new PumpResponseReceivedNotification(message), cancellationToken);
    }
}
