...
class SignalRService {
  ...
  async getBaseUrl() {
    // Check config first
    const configUrl = Config.SIGNALR_BASE_URL || Config.API_BASE_URL;
    if (configUrl) {
  ...
  async start(hubPath = HubPaths.PTS) {
    ...
    if (this.connectionPromise) {
      console.log('[SignalR Mobile] Connection already in progress, waiting...');
  ...
  async _startConnection(hubPath) {
    try {
      ...
      if (this.isTokenExpired(token)) {
        console.warn('[SignalR Mobile] Token is expired');
      ...
      // Create connection with access token factory
      this.connection = new signalR.HubConnectionBuilder()
        ...
        .withAutomaticReconnect({
          nextRetryDelayInMilliseconds: (retryContext) => {
            ...
            console.log(`[SignalR Mobile] Reconnecting in ${delay}ms (attempt ${retryContext.previousRetryCount + 1})`);
            return delay;
          },
        })
  ...
  _setupMessageHandlers() {
    ...
    this.connection.on('UploadStatusUpdate', (data) => {
      console.log('[SignalR Mobile] UploadStatusUpdate received:', data?.deviceId);
  ...
  async subscribeToDevice(deviceId) {
    ...
    try {
      ...
      console.error('[SignalR Mobile] ❌ Failed to subscribe to device:', error);
      return false;
    }
  }
  ...
  async _resubscribeDevices() {
    ...
    console.log('[SignalR Mobile] Re-subscribing to', this.subscribedDevices.size, 'devices');
    ...
    for (const deviceId of this.subscribedDevices) {
      try {
        ...
        console.error('[SignalR Mobile] Failed to re-subscribe to device:', deviceId, error);
      }
    }
  }
  ...
  _handleFuelingEvent(data) {
    ...
    store.dispatch(createFuelingEvent(data.eventType, data.deviceId, data.details));
  ...
  _handleEndOfTransaction(data) {
    ...
    store.dispatch(createFuelingEvent('completed', data.deviceId, {
      transactionId: data.transactionId,
