...
## 🗄️ Database Migration Variants

The PostgreSQL permission seed for the 3-audience rollout now has two variants because live environments do not all use the same Identity table naming.

Use the legacy variant when the database has these tables:
...
```sql
Documentation/Database/Migrations/2026-05-10-three-audience-permissions.sql
```

Use the identity-vnext variant when the database has these tables:
...
```sql
Documentation/Database/Migrations/2026-05-10-three-audience-permissions.identity-vnext.sql
```

Rollback files follow the same split:
...
Rule for future PostgreSQL seed scripts that touch Identity tables:

- If a script inserts into or deletes from role/user Identity tables, ship both a legacy and an `identity-vnext` variant unless the target schema is explicitly fixed for that environment.
