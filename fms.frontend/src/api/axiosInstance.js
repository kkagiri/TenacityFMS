...
const determineApiUrl = async () => {
  if (process.env.NODE_ENV === 'development') {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_PUBLIC_FMS_API_URL}/health`);
      if (response.ok) {
...
const axiosInstance = axios.create({
  baseURL: process.env.REACT_APP_FMS_API_URL_DEV, // Default URL, will be updated
  headers: {
...
axiosInstance.interceptors.request.use(
  async (config) => {
    if (!axiosInstance.defaults.baseURL || axiosInstance.defaults.baseURL === process.env.REACT_APP_FMS_API_URL_DEV) {
