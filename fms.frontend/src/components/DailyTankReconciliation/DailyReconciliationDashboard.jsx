...
import { Card, Row, Col, Table, Alert, Spin, DatePicker, Select, Button, Statistic, Progress } from 'antd';
import {
...
const DailyReconciliationDashboard = () => {
  ...
  const loadDashboardData = async () => {
    setLoading(true);
    try {
      const [reportResponse, summaryResponse, alertsResponse] = await Promise.all([
  ...
  const fetchReconciliationReport = async () => {
    ...
    const response = await fetch(`/api/DailyTankReconciliation/report?${params}`);
  ...
  const fetchReconciliationSummary = async () => {
    ...
    const response = await fetch(`/api/DailyTankReconciliation/summary?${params}`);
    const result = await response.json();
    return result.isSuccess ? result.data : null;
  };
  ...
  const fetchDiscrepancyAlerts = async () => {
    ...
    const response = await fetch(`/api/DailyTankReconciliation/alerts?${params}`);
    const result = await response.json();
  ...
  const processYesterdayReconciliation = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/DailyTankReconciliation/process-yesterday?${params}`, {
        method: 'POST'
      });
      const result = await response.json();

      if (result.isSuccess) {
        message.success(`Successfully processed reconciliation for ${result.data.totalTanksProcessed} tanks`);
        loadDashboardData();
      } else {
  ...
  const getVarianceTrendData = () => {
    ...
    return Object.values(dailyVariances).map(day => ({
      date: day.date,
  ...
  const getTankPerformanceData = () => {
    ...
    const tankVariances = reportData.items.reduce((acc, item) => {
      if (!acc[item.tankName]) {
        acc[item.tankName] = { tankName: item.tankName, totalVariance: 0, count: 0 };
      }
  ...
  const reconciliationColumns = [
    {
      ...
      sorter: (a, b) => moment(a.reconciliationDate).unix() - moment(b.reconciliationDate).unix(),
    },
    {
      title: 'Tank',
      dataIndex: 'tankName',
    ...
    },
    {
      ...
      key: 'variance',
      render: (value, record) => (
        <span className={`tw-font-semibold ${record.hasDiscrepancy ? 'tw-text-red-600' : 'tw-text-green-600'}`}>
          {value.toFixed(2)}
        </span>
      ),
    ...
    },
    {
      ...
      render: (status, record) => (
        <span className={`tw-flex tw-items-center tw-gap-1 ${record.hasDiscrepancy ? 'tw-text-red-600' : 'tw-text-green-600'}`}>
          {record.hasDiscrepancy ? <ExclamationCircleOutlined /> : <CheckCircleOutlined />}
          {status}
        </span>
      ),
    },
  ...
  return (
    <div className="tw-p-6 tw-bg-gray-50 tw-min-h-screen">
      <div className="tw-mb-6">
        ...
        <p className="tw-text-gray-600">Monitor tank reconciliation status and discrepancies</p>
      </div>
      ...
      <Card className="tw-mb-6">
        <Row gutter={16} align="middle">
          <Col span={6}>
            <label className="tw-block tw-text-sm tw-font-medium tw-mb-1">Date Range</label>
            <RangePicker
          ...
          <Col span={4}>
            <label className="tw-block tw-text-sm tw-font-medium tw-mb-1">Site</label>
            <Select
          ...
          <Col span={4}>
            <label className="tw-block tw-text-sm tw-font-medium tw-mb-1">Tank</label>
            <Select
              placeholder="All Tanks"
              value={filters.tankId}
              onChange={(value) => setFilters(prev => ({ ...prev, tankId: value }))}
              className="tw-w-full"
              allowClear
            >
          ...
          <Col span={6}>
            <Card>
              <Statistic
                ...
                prefix={<i className="fa-light fa-database tw-text-blue-500"></i>}
              />
            </Card>
          </Col>
          <Col span={6}>
            <Card>
              <Statistic
                ...
                suffix="%"
                precision={1}
                prefix={<i className="fa-light fa-exclamation-triangle tw-text-orange-500"></i>}
              />
              <Progress
                percent={summary.discrepancyRate}
                showInfo={false}
                strokeColor={summary.discrepancyRate > 10 ? '#ff4d4f' : '#52c41a'}
                className="tw-mt-2"
              />
            </Card>
          </Col>
          <Col span={6}>
            <Card>
              <Statistic
                ...
                suffix="L"
                precision={2}
                prefix={<i className="fa-light fa-chart-line tw-text-green-500"></i>}
              />
            </Card>
          </Col>
          <Col span={6}>
            <Card>
              <Statistic
                ...
                suffix="L"
                precision={2}
                prefix={<i className="fa-light fa-arrow-up tw-text-red-500"></i>}
              />
            </Card>
          </Col>
        </Row>
      ...
      {/* Alerts */}
      {alerts.length > 0 && (
        <Card title={<><WarningOutlined className="tw-mr-2" />Recent Discrepancy Alerts</>} className="tw-mb-6">
          <Row gutter={16}>
      ...
      >
        <Table
          ...
          scroll={{ x: 1200 }}
          size="small"
          rowClassName={(record) => record.hasDiscrepancy ? 'tw-bg-red-50' : ''}
        />
      </Card>
    </div>
  );
