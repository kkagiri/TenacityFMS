...
export const FETCH_HISTORY_CONSUMPTION_FAILURE = 'FETCH_HISTORY_CONSUMPTION_FAILURE';
export const FETCH_CONSUMPTION_BY_ID_REQUEST = 'FETCH_CONSUMPTION_BY_ID_REQUEST';
export const FETCH_CONSUMPTION_BY_ID_SUCCESS = 'FETCH_CONSUMPTION_BY_ID_SUCCESS';
export const FETCH_CONSUMPTION_BY_ID_FAILURE = 'FETCH_CONSUMPTION_BY_ID_FAILURE';
...
export const FETCH_PUMP_TRANSACTIONS_REQUEST = 'FETCH_PUMP_TRANSACTIONS_REQUEST';
export const FETCH_PUMP_TRANSACTIONS_SUCCESS = 'FETCH_PUMP_TRANSACTIONS_SUCCESS';
export const FETCH_PUMP_TRANSACTIONS_FAILURE = 'FETCH_PUMP_TRANSACTIONS_FAILURE';
 ...
 */
export const fetchHistoryConsumptionByVehicle = (vehicleId, date, entry) => async (dispatch) => {
...
export const createConsumption = (consumptionData) => async (dispatch) => {
    ...
    try {
        const response = await axiosInstance.post('/consumption/Create', consumptionData);
 ...
 */
export const fetchConsumptionList = (pagingNo = 200) => async (dispatch) => {
...
export const updateConsumption = (id, consumptionData) => async (dispatch) => {
    ...
    try {
        const response = await axiosInstance.put(`/consumption/Update/${id}`, consumptionData);
 ...
 */
export const importFuelReport = (consumptions, overwriteExisting = false, skipDuplicates = false) => async (dispatch) => {
    ...
    try {
        ...
        const response = await axiosInstance.post('/consumption/import', requestData);
 ...
 */
export const fetchPumpTransactions = (filters = {}) => async (dispatch) => {
    ...
    dispatch({ type: FETCH_PUMP_TRANSACTIONS_REQUEST, filters: serializableFilters });
    ...
    try {
        ...
        if (filters.ptsId) params.append('ptsId', filters.ptsId);
        if (filters.tankId) params.append('tankId', filters.tankId);
        if (filters.startDate) params.append('startDate', filters.startDate.toISOString());
        if (filters.endDate) params.append('endDate', filters.endDate.toISOString());
        if (filters.processedOnly !== undefined) params.append('processedOnly', filters.processedOnly);

        const queryString = params.toString();
        const url = `/consumption/pumptransactions${queryString ? `?${queryString}` : ''}`;
        ...
        if (response.data && response.data.isSuccess) {
            ...
            const errorMessage = response.data.message || 'Failed to fetch pump transactions';
            dispatch({
 ...
 */
export const fetchPumpTransactionsByVehicle = (vehicleId, startDate, endDate) => {
