...
import TankCalibrationLearnedTab from "./TankCalibrationLearnedTab";
import "./TankCalibrationPanel.scss";

const CHART_TYPES = [
  { id: "manual", label: "PTS Manual Chart", icon: "fa-light fa-ruler-vertical" },
  { id: "interval-volume", label: "PTS Interval Volume", icon: "fa-light fa-wave-square" },
  { id: "automatic", label: "PTS Automatic Chart", icon: "fa-light fa-wand-magic-sparkles" },
  { id: "fms-learned", label: "FMS Learned", icon: "fa-light fa-brain-circuit" },
];
...
const TankCalibrationPanel = ({ tank }) => {
  ...
  const isLearnedChart = chartType === "fms-learned";
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      setHistoryRows([]);
      setHistoryTotalCount(0);
      notify(getErrorMessage(error, "Failed to load PTS calibration history."), "error", 4000);
    } finally {
  ...
  const runMutation = async (work, successMessage) => {
    ...
    try {
      ...
      notify(getErrorMessage(error, "PTS calibration operation failed."), "error", 4000);
      return null;
    } finally {
      setSubmitting(false);
    }
  ...
  const handleGenerateAutomatic = async () => {
    await runMutation(
      ...
      "PTS automatic calibration chart generated and stored locally."
    );
  };

  const automaticCalibrationBlockedReason =
    chartType !== "automatic"
      ? null
      : healthSummary?.automaticCalibrationEnabled === false
        ? healthSummary?.automaticCalibrationConfigurationMessage || "PTS automatic calibration is disabled in PTS tank configuration."
        : healthSummary?.automaticCalibrationEnabled === true && healthSummary?.automaticCalibrationReadyForGeneration === false
          ? healthSummary?.automaticCalibrationConfigurationMessage || "PTS automatic calibration is enabled, but the controller is not ready for generation yet."
          : null;
  ...
  const handleSetChart = async () => {
    ...
    await runMutation(
      ...
      "PTS manual calibration chart replaced on the controller."
    );
  };
  ...
  const handleAddDirect = async () => {
    ...
    await runMutation(
      ...
      `PTS manual calibration record added at ${normalizedRecord.height} mm.`
    );
  };
  ...
  const handleUpdateDirect = async () => {
    ...
    await runMutation(
      ...
      `PTS manual calibration record updated at ${normalizedRecord.height} mm.`
    );
  };
  ...
  const handleDeleteDirect = async () => {
    ...
    await runMutation(
      ...
      `PTS manual calibration record deleted at ${height} mm.`
    );
  };
  ...
  const handleViewGraph = () => {
    if (!canViewGraph) {
      notify("Sync or load at least two PTS calibration rows to view the graph.", "warning", 3000);
      return;
    }
    ...
    <div className="m365-tank-calibration">
      <div className="m365-tank-calibration__header">
        <div>
          <h2 className="m365-tank-calibration__title">
            <i className={`fa-light ${isLearnedChart ? "fa-brain-circuit" : "fa-flask-vial"}`}></i>
            {isLearnedChart ? "FMS Learned Calibration" : "PTS Calibration Workspace"}
          </h2>
          <p className="m365-tank-calibration__subtitle">
            {isLearnedChart
              ? `FMS-side learned calibration, coverage, seeding, and comparison for ${tank.name}.`
              : `Local PTS calibration history, PTS sync, and controller chart management for ${tank.name}.`}
          </p>
        </div>
        <div className="m365-tank-calibration__binding">
          ...
          <span className="m365-badge m365-badge--neutral">PTS Tank {tank.ptsTankId || "-"}</span>
          {isLearnedChart && <span className="m365-badge m365-badge--success">FMS Learned</span>}
        </div>
      </div>

      {!hasPtsBinding && !isLearnedChart && (
        <div className="m365-tank-calibration__empty">
          Link this tank to a PTS device and probe before using PTS calibration sync or chart editing.
        </div>
      )}
      ...
      <>
        <div className="m365-tank-calibration__tab-bar">
          ...
          {!isLearnedChart && hasPtsBinding && (
            <div className="m365-tank-calibration__actions">
              <button type="button" className="m365-btn m365-btn--ghost" onClick={() => setHelpOpen(true)} title="PTS Calibration Help">
              ...
              {chartType === "automatic" && (
                <button
                  ...
                  disabled={submitting || loadingHealth || Boolean(automaticCalibrationBlockedReason)}
                  title={automaticCalibrationBlockedReason || "Generate a PTS automatic calibration chart on the controller."}
                >
                  <i className="fa-light fa-wand-magic-sparkles"></i>
                  Generate PTS Automatic
                </button>
              )}
              {isManualChart && (
        ...
        {isLearnedChart ? (
          <TankCalibrationLearnedTab tank={tank} />
        ) : hasPtsBinding && (
          <>
            ...
            {chartType === "automatic" && automaticCalibrationBlockedReason && (
              <div className="m365-tank-calibration__recommendation">
                <i className="fa-light fa-triangle-exclamation"></i>
                <div>
                  <strong>PTS Automatic Generation Unavailable</strong>
                  <p>{automaticCalibrationBlockedReason}</p>
                </div>
              </div>
            )}

            {/* ── PTS Calibration Health & Variance Analysis ── */}
              ...
              {healthSummary ? (
                <>
                  <div className="m365-tank-calibration__health-grid">
                    ...
                    <div className="m365-tank-calibration__health-card">
                      <span className="m365-tank-calibration__health-label">PTS Auto Calibration</span>
                      <strong className="m365-tank-calibration__health-value">
                        {healthSummary.automaticCalibrationEnabled === true
                          ? healthSummary.automaticCalibrationReadyForGeneration === true
                            ? "Enabled / Ready"
                            : "Enabled / Not Ready"
                          : healthSummary.automaticCalibrationEnabled === false
                            ? "Disabled"
                      ...
                      <span className="m365-tank-calibration__health-meta">
                        {healthSummary.automaticCalibrationConfigurationMessage || "PTS configuration not checked."}
                      </span>
                    </div>
                  </div>
                ...
                <div className="m365-tank-calibration__empty">
                  {loadingHealth ? "Analysing PTS calibration health..." : "No PTS calibration health data available. Sync PTS calibration data and record deliveries to generate analysis."}
                </div>
              )}
            </div>
            ...
            {isManualChart && (
              <div className="m365-tank-calibration__editor-card">
                <div className="m365-tank-calibration__editor-header">
                  <div>
                    <h3>PTS Manual Chart Editor</h3>
                    <p>Work on a local PTS manual-chart draft, then push the full chart to the controller, or apply direct add/edit/delete operations.</p>
                  </div>
                  <div className="m365-tank-calibration__editor-actions">
                    <button type="button" className="m365-btn m365-btn--sm m365-btn--ghost" onClick={() => setHelpOpen(true)} title="PTS Calibration Help">
                      <i className="fa-light fa-circle-question"></i>
                    </button>
            ...
            {canViewGraph && (
              <div ref={graphSectionRef} className="m365-tank-calibration__panel-card">
                <div className="m365-tank-calibration__section-heading">
                  <h3><i className="fa-light fa-chart-line"></i> PTS Calibration Curve</h3>
            ...
            <div className="m365-tank-calibration__content-grid">
              ...
              <div className="m365-tank-calibration__panel-card">
                ...
                <DataGrid
                  ...
                  showRowLines
                  hoverStateEnabled
                  noDataText="No PTS calibration history stored yet"
      ...
      >
        <div className="m365-tank-calibration__help">
          ...
          <section className="m365-tank-calibration__help-section">
            ...
            <dl>
              <dt>Sync From PTS</dt>
              <dd>Reads the currently active PTS calibration chart from the physical controller or probe and stores a copy in the local database. This creates a new history snapshot. Use this to keep your local records in sync with what is actually on the device.</dd>
              <dt>Set Controller Chart <span className="m365-badge m365-badge--info">PTS manual only</span></dt>
              <dd>Pushes your entire draft chart (all rows in the Draft Rows grid) to the PTS controller, replacing whatever PTS chart is currently on the device. After the push, a sync is performed automatically so the local snapshot matches.</dd>
              <dt>Generate PTS Automatic <span className="m365-badge m365-badge--info">PTS automatic only</span></dt>
          ...
          <section className="m365-tank-calibration__help-section">
            <h4><i className="fa-light fa-pen-ruler"></i> PTS Manual Chart Editor</h4>
            <p>Two workflows are available for PTS manual charts:</p>
          ...
          <section className="m365-tank-calibration__help-section">
            <h4><i className="fa-light fa-heart-pulse"></i> PTS Calibration Health</h4>
            <dl>
              <dt>How is it calculated?</dt>
              <dd>The system compares recent fuel deliveries against the active PTS calibration chart. For each delivery, it calculates the difference between the volume change measured by the probe and the actual dispensed amount. The average of these variances determines the overall quality rating:</dd>
            </dl>
            <ul>
              ...
              <li><span className="m365-badge m365-badge--danger">Poor</span> — Average variance &gt; 5% (PTS recalibration recommended)</li>
          ...
          <section className="m365-tank-calibration__help-section">
            ...
            <dl>
              ...
              <dd>The most recent copy of the selected PTS calibration chart stored in the local database. If this is empty, click "Sync From PTS" to pull the chart from the device. Click any row to load it into the editor inputs (manual chart only).</dd>
              <dt>Local History</dt>
              <dd>Every time a sync or chart push happens, a timestamped snapshot is saved. History lets you review past PTS chart versions, compare changes, and audit who modified the chart and when. Each chart type (manual, interval volume, automatic) has its own history trail.</dd>
