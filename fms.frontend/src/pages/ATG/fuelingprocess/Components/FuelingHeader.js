...
const FuelingHeader = ({
    ...
    if (ptsDevice?.ptsid || ptsDevice?.deviceId) {
      ...
      if (savedTankId) {
        ...
        if (!isNaN(parsedTankId)) {
          console.log(
            "[FuelingHeader] Restored tank selection from localStorage:",
            parsedTankId
          );
  ...
  useEffect(() => {
    if (selectedTankId && (ptsDevice?.ptsid || ptsDevice?.deviceId)) {
      ...
      console.log(
        "[FuelingHeader] Saved tank selection to localStorage:",
        selectedTankId
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const loadTanks = async () => {
      ...
      try {
        ...
        if (selectedTankId) {
          ...
          if (!tankExists) {
            console.log(
              "[FuelingHeader] Previously selected tank no longer exists, clearing selection"
            );
  ...
  const isConnectionUnstable =
    connectionStatus === "delayed" || connectionStatus === "connecting";
          ...
          <div className="header-left">
            ...
            <div className="header-info">
              <h2 className="tw-text-lg md:tw-text-xl lg:tw-text-2xl tw-font-semibold tw-mb-1">
                {ptsDevice.ptsName || siteName || ptsDevice.name}
              </h2>
                ...
                <Button
                  ...
                  text={
                    activeFuelingProcesses && activeFuelingProcesses.length > 0
                      ? `Active (${activeFuelingProcesses.length})`
                      : "Active"
                  }
                  type="default"
                  stylingMode="outlined"
        ...
        <div className="header-bottom-row">
          ...
          <div className="header-tank-section">
            <div className="tank-selector-container">
              ...
              {isLoadingTanks ? (
                <div
                  className="tw-flex tw-items-center tw-gap-2 tw-p-3 tw-bg-gray-50 tw-rounded tw-border tw-border-gray-200"
                  key="loading"
                >
                  <i
                    className="fa-light fa-spinner fa-spin tw-text-blue-500"
                    key="spinner-icon"
                  ></i>
                ...
                <div key="tank-select-wrapper">
                  <SelectBox
                    ...
                    itemRender={(tank) => (
                      <div
                        className="tw-flex tw-items-center tw-justify-between tw-py-1"
                        key={`tank-item-${tank.id}`}
                      >
                        <div className="tw-flex-1">
                          ...
                          <div className="tw-text-xs tw-text-gray-500">
                            {tank.currentLevel?.toLocaleString() || 0}L /{" "}
                ...
                <div
                  className="tw-flex tw-flex-col tw-gap-2 tw-p-3 tw-bg-yellow-50 tw-rounded tw-border tw-border-yellow-200"
                  key="no-tanks"
                >
                  <div className="tw-flex tw-items-center tw-gap-2">
                    <i
                      className="fa-light fa-triangle-exclamation tw-text-yellow-600"
                      key="warning-icon"
                    ></i>
            ...
            {selectedTankId &&
              tanksWithProbeData &&
              tanksWithProbeData.length > 0 &&
              (() => {
                const selectedTank = tanksWithProbeData.find(
                      ...
                      <div className="tw-flex tw-flex-col">
                        ...
                        >
                          ...
                          {selectedTank.probeOnline && (
                            <i
                              className="fa-light fa-signal tw-text-green-500 tw-ml-1 tw-text-xs"
                              title="Real-time"
                            ></i>
                          )}
                        </span>
      ...
      {isDeviceDisconnected && (
        ...
        >
          ...
          <span className="tw-block sm:tw-inline tw-ml-2">
            Fueling operations are disabled until connection is restored. The
            system will automatically reconnect when the device comes back
            online.
          </span>
        </div>
      )}
      ...
      {!isDeviceDisconnected &&
        ...
        activeFuelingProcesses?.some((p) => p.status === "fueling") && (
          ...
          >
            ...
            <span className="tw-block sm:tw-inline tw-ml-2">
              Brief connection issues detected. Active fueling will continue,
              but if connection doesn't stabilize within 60 seconds, operations
              may be affected.
            </span>
          </div>
        )}
