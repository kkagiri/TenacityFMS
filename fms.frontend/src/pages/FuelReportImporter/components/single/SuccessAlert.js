...
const SuccessAlert = ({ importSuccessInfo, setImportSuccessInfo }) => {
  ...
  // Extract record count from multiple possible sources
  const recordCount =
    importSuccessInfo.successCount ||
    importSuccessInfo.consumptions?.length ||
    importSuccessInfo.totalProcessed ||
    0;
        ...
        <div className="tw-flex-1">
          ...
          <div className="tw-text-green-700">
            <div className="tw-mb-2">
              {importSuccessInfo.message ||
                `Successfully imported ${recordCount} consumption records.`}
            </div>

            <div className="tw-bg-white tw-rounded-md tw-p-3 tw-border tw-border-green-200 tw-mb-2">
              <div className="tw-flex tw-items-center tw-gap-2 tw-mb-1">
                <i className="fa-solid tw-file-invoice tw-text-green-600"></i>
              ...
              {recordCount > 0 && (
                <div className="tw-flex tw-items-center tw-gap-2">
                  ...
                  <span className="tw-text-sm">
                    <strong>{recordCount}</strong> record
                    {recordCount !== 1 ? "s" : ""} imported
                  </span>
                </div>
              )}
            </div>
