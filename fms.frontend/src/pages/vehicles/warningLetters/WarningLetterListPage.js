...
const WarningLetterListPage = () => {
    ...
    const renderStatus = ({ value }) => {
        const item = statusMap[value] || { label: value || "Unknown", cls: "m365-badge--neutral" };
    ...
    const renderActions = ({ data }) => (
        <div className="warning-letter-list__actions m365-btn-group">
            <button type="button" className="m365-btn m365-btn--ghost" onClick={() => navigate(`/reports/warning-letters/${data.id}/preview`)}>
                Preview
            </button>
            {canUpdate && data.status === 0 && (
                <button type="button" className="m365-btn m365-btn--ghost" onClick={() => navigate(`/reports/warning-letters/${data.id}/edit`)}>
                    Edit
                </button>
            )}
            {canFinalize && data.status === 0 && (
                <button type="button" className="m365-btn m365-btn--primary" onClick={() => handleFinalize(data)}>
                    Finalize
                </button>
            )}
            {canUpdate && data.status !== 0 && data.status !== 3 && (
                <button type="button" className="m365-btn m365-btn--success" onClick={() => handleAcknowledge(data)}>
                    Acknowledge
                </button>
            )}
            {canDelete && data.status === 0 && (
                <button type="button" className="m365-btn m365-btn--danger" onClick={() => handleDelete(data)}>
                    Delete
                </button>
            )}
        </div>
        ...
        <div className="warning-letter-page warning-letter-list">
            ...
            <div className="warning-letter-page__panel warning-letter-list__grid">
                ...
                >
                    ...
                    <Column caption="Actions" width={360} allowSorting={false} allowFiltering={false} cellRender={renderActions} />
                </DataGrid>
            </div>
        </div>
    );
