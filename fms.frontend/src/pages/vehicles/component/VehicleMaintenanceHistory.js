...
const VehicleMaintenanceHistory = ({ vehicleId }) => {
  ...
  const loadMaintenanceHistory = useCallback(async (forceReload = false) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const processedData = (response.data || []).map(item => ({
          ...
          nextServiceDue: item.nextServiceDue ? new Date(item.nextServiceDue) : null
        }));
  ...
  }, [dispatch, vehicleId, dateFrom, dateTo, statusFilter]);

  useEffect(() => {
    if (vehicleId && !dataLoadedRef.current) { // Only load if not already loaded
      loadMaintenanceHistory();
    }
  }, [vehicleId, loadMaintenanceHistory]);
  ...
  const handleAddMaintenance = async () => {
    try {
      ...
      if (response.success) {
        notify('Record added successfully', 'success', 3000);
