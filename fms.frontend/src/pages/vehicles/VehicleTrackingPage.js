        ...
        <div className="tw-w-80 tw-bg-white tw-border-r tw-flex tw-flex-col">
          ...
          <div className="tw-flex-1 tw-overflow-y-auto">
            {filteredVehicles.map((vehicle) => (
              <div
                key={vehicle.id}
                className={`tw-p-3 tw-border-b tw-cursor-pointer tw-transition-colors hover:tw-bg-gray-50 ${
                  selectedVehicle?.id === vehicle.id ? 'tw-bg-blue-50 tw-border-l-4 tw-border-l-blue-500' : ''
                }`}
                onClick={() => handleVehicleClick(vehicle)}
              >
                <div className="tw-flex tw-items-center tw-justify-between">
                  <span className="tw-font-medium tw-text-gray-800">{vehicle.name || 'Unknown'}</span>
                  <span className={`tw-px-2 tw-py-0.5 tw-rounded tw-text-xs ${
                    (vehicle.speed ?? 0) > 0 ? 'tw-bg-green-100 tw-text-green-700' : 'tw-bg-gray-100 tw-text-gray-600'
                  }`}>
