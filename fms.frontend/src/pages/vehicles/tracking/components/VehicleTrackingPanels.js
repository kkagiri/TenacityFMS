...
export const VehicleTrackingSidebarPanel = React.memo(({
  ...
  return (
    <div className="vehicle-tracking-sidebar tw-flex tw-h-full tw-min-h-0 tw-flex-col tw-bg-white">
      <div className="vehicle-tracking-sidebar__search tw-space-y-2 tw-border-b tw-p-3">
        <div className="vehicle-tracking-sidebar__summary tw-flex tw-items-center tw-justify-between tw-text-xs tw-text-gray-500">
          <span>Vehicle grid</span>
          <span>{gridFilteredVehiclesCount} of {filteredVehiclesCount}</span>
        </div>
        {Array.isArray(clusterFilteredVehicleIds) && clusterFilteredVehicleIds.length > 0 && (
          <div className="vehicle-tracking-sidebar__cluster-filter tw-flex tw-items-center tw-justify-between tw-gap-3 tw-rounded-md tw-border tw-border-blue-200 tw-bg-blue-50 tw-px-3 tw-py-2 tw-text-xs tw-text-blue-800">
            <span>
              Cluster filter active: showing {gridFilteredVehiclesCount} vehicle{gridFilteredVehiclesCount === 1 ? '' : 's'}
            </span>
            <button
              type="button"
              onClick={clearClusterFilter}
              className="vehicle-tracking-sidebar__cluster-filter-clear tw-inline-flex tw-items-center tw-gap-1 tw-rounded-md tw-border tw-border-blue-300 tw-bg-white tw-px-2 tw-py-1 tw-font-medium tw-text-blue-700 hover:tw-bg-blue-100"
            >
      ...
      <div className="vehicle-tracking-grid tw-flex-1 tw-min-h-0">
        <DataGrid
          ...
          onOptionChanged={handleTrackingGridOptionChanged}
        >
          <Selection mode="single" />
          <Sorting mode="multiple" />
          <SearchPanel visible={true} width={260} placeholder="Search vehicles..." text={searchText} />
          ...
          <Column dataField="trackingCode" caption="Hyoung No" minWidth={110} />
          <Column
            dataField="lastSeenAt"
            caption="Last seen"
            dataType="datetime"
            minWidth={150}
            customizeText={({ value }) => formatTrackingTimestamp(value)}
          />
          <Column
          ...
          />
          <Column dataField="driverNameLabel" caption="Driver Name" minWidth={160} />
          <Column
          ...
          />
          <Column dataField="plateNumberLabel" caption="Plate No" minWidth={120} visible={false} />
          <Column
            ...
            visible={false}
          />
          <Column dataField="ignitionLabel" caption="Ignition" minWidth={90} visible={false} />
          <Column dataField="addressLabel" caption="Location" minWidth={220} visible={false} />
        </DataGrid>
      </div>
    </div>
  );
