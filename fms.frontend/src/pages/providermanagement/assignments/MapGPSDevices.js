...
import { Column, Paging, Pager, Selection, Scrolling, SearchPanel } from "devextreme-react/data-grid";
...
const MapGPSDevices = () => {
  ...
  const filteredDevices = useMemo(() => {
    ...
    return filtered;
  }, [devices, showOnlineOnly, showUnmappedOnly, mappedDeviceIds]);
  ...
  const filteredVehicles = useMemo(() => {
    ...
    if (showUnmappedOnly) {
      ...
      filtered = filtered.filter(v => !devices.some(d => d.mappedVehicleId === v.vehicleId && d.isMapped));
    }
    return filtered;
  ...
  const mapSelectedDeviceToVehicleId = useCallback(async (vehicleId) => {
    if (!selectedDevice) {
  ...
  const handleVehicleSelectionChanged = useCallback(async (e) => {
    ...
    if (selectedDevice && !mapping) {
      ...
      if (result) {
        ...
        setMapping(true);
        try {
          const success = await mapSelectedDeviceToVehicleId(vehicle.vehicleId);
          if (success) {
  ...
  }, [selectedDevice, mapping, mapSelectedDeviceToVehicleId]);  // Execute mapping for selected device and vehicle (fallback button)
  const handleMapDevice = useCallback(async () => {
    ...
    setMapping(true);
    try {
      const success = await mapSelectedDeviceToVehicleId(selectedVehicle.vehicleId);
    ...
    <div className="map-gps-devices tw-h-full tw-flex tw-flex-col tw-p-6">
          ...
          <div className="tw-flex tw-justify-between tw-items-center">
            <div>
              ...
              <p className="tw-text-sm tw-text-gray-600 tw-mt-1">
                Select a GPS device from your tracking provider and map it to an FMS vehicle
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="tw-flex-1 tw-grid tw-grid-cols-2 tw-gap-6 tw-overflow-hidden tw-min-h-0">
            ...
            <h3 className="tw-font-semibold tw-text-gray-800 tw-text-base">
              ...
              {selectedProvider && (
                <span className="tw-text-sm tw-font-normal tw-text-gray-500 tw-ml-2">
                  from {providers.find(p => p.providerName === selectedProvider)?.displayName || selectedProvider}
                </span>
              )}
            </h3>
            {!selectedDevice ? (
        ...
        >
          ...
          <div className={`tw-flex-1 tw-overflow-hidden ${!selectedDevice ? 'tw-pointer-events-none' : ''}`}>
            <DataGrid
              ref={vehicleGridRef}
              ...
              onSelectionChanged={selectedDevice ? handleVehicleSelectionChanged : undefined}
              onFocusedRowChanged={(e) => {
        ...
        <div className="tw-flex tw-flex-col tw-gap-4">
              ...
              <div className="tw-flex tw-items-start tw-gap-3">
                ...
                <div>
                  <div className="tw-text-sm tw-text-blue-900">
                    <span className="tw-font-semibold">Device:</span> {selectedDevice.name} (IMEI: {selectedDevice.imei})
                  </div>
                  <div className="tw-text-sm tw-text-blue-800">
                    <span className="tw-font-semibold">Vehicle:</span> {selectedVehicle.hyoungNo} ({selectedVehicle.numberPlate})
                  </div>
                </div>
              </div>
              <Button
          ...
          {!selectedDevice && !selectedVehicle && (
            <div className="tw-flex tw-items-center tw-gap-2 tw-text-sm tw-text-gray-500 tw-p-2">
              ...
              <span><strong>Step 1:</strong> Select a device from the left panel</span>
            </div>
          )}
          ...
          {selectedDevice && !selectedVehicle && (
            <div className="tw-flex tw-items-center tw-gap-2 tw-text-sm tw-text-green-600 tw-p-2 tw-bg-green-50 tw-rounded">
              ...
              <span><strong>Device selected:</strong> {selectedDevice.name} — Now select a vehicle from the right panel</span>
            </div>
          )}
