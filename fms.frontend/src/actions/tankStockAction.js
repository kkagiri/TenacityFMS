...
export const createOpeningStock = (tankId, amount) => async (dispatch) => {
  try {
    const response = await axiosInstance.post(`/tankstock/openingstock?tankId=${tankId}&amount=${amount}`);
    dispatch({ type: CREATE_OPENING_STOCK_SUCCESS, payload: response.data });
  } catch (error) {
    dispatch({ type: CREATE_OPENING_STOCK_FAILURE, payload: error.message });
  }
};

export const createClosingStock = (tankId, amount) => async (dispatch) => {
  try {
    const response = await axiosInstance.post(`/tankstock/closingstock?tankId=${tankId}&amount=${amount}`);
