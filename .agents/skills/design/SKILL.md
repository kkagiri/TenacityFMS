...
## 2.5 Layout Architecture — Two-Tier Standard

> 🚨 **STANDARD:** FMS uses **two and only two** application shells. Every page must mount inside one of them. Do not invent additional layout wrappers.

### Tier 1 — Main App Shell (Inspinia)

The default shell for all feature applications (Dashboard, Vehicles, Tank Stock, Fuel, Reports, etc.).

| Aspect            | Detail                                                                                        |
| ----------------- | --------------------------------------------------------------------------------------------- |
| Wrapper component | `fms.frontend/src/layouts/side-nav-outer-toolbar/side-nav-outer-toolbar.js`                   |
| Side menu         | `fms.frontend/src/components/side-navigation-menu/SideNavigationMenu.js`                      |
| Header            | `fms.frontend/src/components/header/Header.js`                                                |
| Menu source       | Backend `navigationitems` table via `fetchNavigationItems` redux action — permission-filtered |
| Theme             | Inspinia dark sidebar (`#2f4050` → `#293846` gradient), accent `#1ab394`, white topbar        |
| Mounted in        | `fms.frontend/src/Content.js` (default branch)                                                |

**Inspinia tokens:**

| Token               | Hex       | Usage                                |
| ------------------- | --------- | ------------------------------------ |
| Sidebar bg start    | `#2f4050` | Top of sidebar gradient              |
| Sidebar bg end      | `#293846` | Bottom of sidebar gradient           |
| Sidebar text        | `#a7b1c2` | Default menu item text               |
| Sidebar text active | `#ffffff` | Hover/selected text                  |
| Accent (selection)  | `#1ab394` | Selected item left bar, primary CTAs |
| Secondary accent    | `#1c84c6` | Links, info actions                  |
| Content bg          | `#f3f3f4` | Outer content scroll area            |
| Topbar bg           | `#ffffff` | Header surface                       |
| Topbar border       | `#e7eaec` | Header bottom border                 |
| Topbar height       | `56px`    | Fixed                                |

### Tier 2 — Admin Shell (M365)

The dedicated full-page shell for `/admin/*` routes. Renders **outside** the Inspinia shell.

| Aspect            | Detail                                                                                 |
| ----------------- | -------------------------------------------------------------------------------------- |
| Wrapper component | `fms.frontend/src/pages/admin/layout/AdminLayout.js`                                   |
| Menu source       | Hardcoded `adminRoutes` from `fms.frontend/src/pages/admin/utils/navigationHelper.js`  |
| Theme             | M365 light — see Section 2 tokens above                                                |
| Mounted in        | `fms.frontend/src/Content.js` early-return branch when `pathname.startsWith('/admin')` |

**Why a separate shell:** admin is a self-contained management area with its own grouped navigation (Users, Roles, Permissions, Sites, Tanks, Notifications, etc.). Nesting it inside the Inspinia drawer would produce a confusing dual-sidebar UX.
...
if (location.pathname.startsWith("/admin")) {
  // Bare mount — AdminLayout owns the chrome
  return <Routes>{/* /admin and /admin/* */}</Routes>;
}
...
| ✅ DO                                                                                   | ❌ DON'T                                                             |
| --------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Mount feature pages inside `SideNavOuterToolbar`                                        | Mount feature pages bare or inside `AdminLayout`                     |
| Mount `/admin/*` pages bare (AdminLayout handles chrome)                                | Wrap admin pages in `SideNavOuterToolbar` (causes nested sidebars)   |
| Add new modules via the backend `navigationitems` table (Admin → Navigation Management) | Hardcode module entries in the sidebar component                     |
| Use `Header.js` only inside Inspinia shell                                              | Reintroduce the `app-drawer` launcher or `AppDrawerLayout`           |
| Add new admin pages to `adminRoutes` in `navigationHelper.js`                           | Add admin pages to backend `navigationitems` (those are Tier 1 only) |

### Deprecated — Do Not Use

The following are removed from the active code path. Do **not** import, reference, or reintroduce them:

- `fms.frontend/src/components/app-drawer/` — module launcher (replaced by Inspinia sidebar)
- `fms.frontend/src/layouts/app-drawer-layout/` — old shell wrapper

The `Header.js` component no longer exposes app-launcher mode; it always operates as the sidebar toggle for `SideNavOuterToolbar`.
...
<div className="m365-info-banner">
  ...
  <div className="m365-info-banner__content">
    ...
    <a href="#" className="m365-info-banner__link">
      Learn more
    </a>
  </div>
...
_FMS Design System · M365 Admin Center · v1.0_
