...
public class TagMonitoringController : ControllerBase {
    private readonly IMediator _mediator;
    private readonly GpsdataContext _context;
    public TagMonitoringController (IMediator mediator, GpsdataContext context) {
        _mediator = mediator;
        _context = context;
    }
    ...
    [HttpPost ("config")]
    public async Task<IActionResult> CreateConfig ([FromBody] FMS.Domain.Entities.TagMonitoringConfig config) {
        var result = await _mediator.Send (new CreateTagMonitoringConfigCommand (config));
        return Ok (result);
    }

    [HttpPut ("config/{id}")]
    public async Task<IActionResult> UpdateConfig (int id, [FromBody] FMS.Domain.Entities.TagMonitoringConfig config) {
