 ...
 *
 * Key Sections:
 * - Report header with date range and filter summary
 * - Summary cards for transaction totals, volume, amount, and unique vehicles
 * - Transaction detail table sourced from pumptransaction records
 * - Empty-state fallback and report footer
 */
namespace FMS.WebClient.Services.Reporting
{
    /// <summary>
    /// Dedicated HTML template for the Pump Transaction report.
    /// </summary>
    internal static class PumpTransactionHtmlTemplate
    {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .summary-card.card-warning::before { background: #D97706; }
        ...
        .data-table tfoot td { background: #F9FAFB; font-weight: 800; color: var(--text-strong); }
...
<div class=""page"">
    <div class=""report-header"">
        <div class=""report-title""><h1>{{reportTitle}}</h1><p>{{reportSubtitle}}</p></div>
    ...
    <div class=""filter-summary"">
        ...
        <div class=""filter-grid"">
            ...
            {{#if vehicleName}}<div class=""filter-item""><span class=""filter-label"">Vehicle:</span><span class=""filter-value"">{{vehicleName}}</span></div>{{/if}}
            {{#if fuelGrade}}<div class=""filter-item""><span class=""filter-label"">Fuel Grade:</span><span class=""filter-value"">{{fuelGrade}}</span></div>{{/if}}
        </div>
    </div>
        ...
        <table class=""data-table"">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class=""font-bold"">{{ptsName}}</span><br /><span class=""text-muted"">{{pump}}</span></td>
                    <td class=""font-bold"">{{vehicleNumberPlate}}</td>
            ...
            <tfoot>
                <tr>
                    ...
                    <td class=""text-right"">{{summary.avgConsumption}}</td>
    ...
    {{#unless transactions}}
    <div class=""empty-state""><p>No pump transactions found for the selected criteria.</p></div>
    {{/unless}}
